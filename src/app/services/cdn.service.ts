...
export class CDNService {
    ...
    uploadFeedApi(value: CDNFile, info: any) {
        const formData: FormData = new FormData();
        formData.append('file', value.file, value.file.name);
        formData.append('info', JSON.stringify(
            info
        ));
        return this.http.post( environment.feedApi + 'cdn/' + 'upload', formData);
    }
            ...
            setTimeout(() => {
                if (file) {
                    ...
                    this.uploadFile(formData).subscribe(response => {
                        ...
                        e => {
                            if (e.error.message.includes('FILE_DUPLICATE')) {
                                alert('O arquivo já existe...');
                            }
                            if (e.error.message.includes('FILE_MAX_SIZE')) {
