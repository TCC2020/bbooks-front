...
export class ConsultaCepService {
    ...
    getStates(idCountry: string): Observable<State[]> {
        return this.http.get<any>('https://secure.geonames.org/childrenJSON?geonameId=' + idCountry + '&username=' + environment.apicep) // bull2020 por environment.apicep
            .pipe(
                map(response => {
    ...
    getCountry(): Observable<Country[]> {
        return this.http.get<any>('https://secure.geonames.org/countryInfoJSON?lang=pt&username=' + environment.apicep) // trocar bulls2020 por environment.apicep
            .pipe(
                map(response => {
                    const t = response.geonames.map(result => {
                        const c = {
